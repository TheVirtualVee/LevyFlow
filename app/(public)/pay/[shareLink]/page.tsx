...
export default function PaymentPage({ params }: { params: { shareLink: string } }) {
  ...
  async function handleSubmit(formData: FormData) {
    ...
    try {
      ...
      // Inject validated token details into payload for verification mapping
  ...
  // SUCCESS SCREEN (AUTO-VERIFIED - SHOW DECENTRALIZED CLIENT-HELD QR)
  if (status === 'success') {
    return (
  ...
  // FAILED SCREEN (OCR UNABLE TO VERIFY - DISPUTE OPTION)
  if (status === 'failed') {
    return (
      <div className="min-h-screen flex items-center justify-center bg-slate-950 px-4">
  ...
  // DISPUTED SCREEN (MANUAL REVIEW CONFIRMED)
  if (status === 'disputed') {
    return (
      <div className="min-h-screen flex items-center justify-center bg-slate-950 px-4">
        <div className="max-w-md w-full bg-slate-900 border border-slate-800 rounded-2xl shadow-2xl p-8 text-center backdrop-blur-lg">
  ...
  // GATE CLEARED - RENDER ACTIVE PAYMENT INPUT FORM
  return (
    <>
