...
export async function GET(req: NextRequest) {
  // Simple header authorization using Vercel Cron Secret (if configured)
  ...
  try {
    ...
    // Select campaigns that are past their ends_at date by 7 days and not yet archived
    ...
    if (campaigns && campaigns.length > 0) {
      for (const campaign of campaigns) {
        try {
          ...
          console.error(`Failed to auto-archive campaign ${campaign.id}:`, archiveErr.message)
        }
      }
    }
    ...
    console.error('Auto-archive cron execution error:', err.message)
