...
export function validateMatricNumber(matric: string, pattern?: string | null): boolean {
  ...
  if (pattern) {
    try {
      ...
      console.error('Invalid matric validation regex pattern:', pattern, e)
    }
  }

  // Default fallback: allow alphanumeric, hyphens, and slashes.
  // E.g., "EEG/2018/042", "180405012", "FUTA-19-1234"
...
export function validateContactInfo(contact: string): boolean {
  ...
  const phoneRegex = /^(?:\+234|0)[789][01]\d{8}$/ // Nigerian phone numbers
