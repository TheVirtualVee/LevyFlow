...
export function PaymentReceiptQR({ 
  ...
  // Construct compressed data format to maximize scan speed
  const paymentData = {
  ...
  useEffect(() => {
    // Render QR Code in browser - zero server workload or database pressure
    QRCode.toDataURL(JSON.stringify(paymentData), {
      width: 250,
      margin: 2,
      errorCorrectionLevel: 'H', // High error correction redundancy
      color: {
